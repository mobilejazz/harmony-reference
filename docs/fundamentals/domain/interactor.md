...
In order to access the CRUD operations of a [`Repository`](/docs/fundamentals/data/repository/concepts), there are interactors for each CRUD operation.
...
Similar to the [`Repository`](/docs/fundamentals/data/repository/concepts) public interface, all default interactors interfaces are extended with methods to access the CRUD functions by an Id using the `IdQuery`.

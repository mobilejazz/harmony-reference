...
Note that a `Query` must be independent of its data source. Calling a query `MyNetworkActionQuery` is wrong (use instead `MyActionQuery`) as queries must be abstracted from its source and can be potentially used in any [`DataSource`](data-source).
...
Queries must be pro-actively supported in each [`DataSource`](data-source) implementation. A typical appearance of an implemented `get` method from a `GetDataSource` would be:
...
In order to create a key-value environment for data sources as in [`InMemoryDataSource<T>`](in-memory-data-source), [`DeviceStorageDataSource<T>`](device-storage-data-source) or any custom implementation, there is the `KeyQuery` interface/protocol to implement:
